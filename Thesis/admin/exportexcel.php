...
if($_POST) {
	...
	$start_date = $_POST['startDates'];
	$end_date = $_POST['endDates'];
    ...
    $totalIncome =0;
      ...
      $cumul = $result['Transaction_grandtotal'] - $result['Transaction_origtotal'];
        	...
        	<td><center>'.'php'.' '.$result['Transaction_subtotal'].'</center></td>
						<td><center>'.'php'.' '.$ships.'</center></td>
        <td><center>'.'php'.' '.$result['Transaction_origtotal'].'</center></td>

        <td><center>'.'php'.' '.$cumul.'</center></td>
			</tr>';
      ...
      $totalIncome += ($result['Transaction_grandtotal'] - $result['Transaction_origtotal']);
		...
		<tr>
			...
			<td colspan="2"><center>'.'php'.' '.$totalAmount.'</center></td>
      <td><center>'.'php'.' '.$totalOrig.'</center></td>
      <td><center>'.'php'.' '.$totalIncome.'</center></td>
		</tr>
	</table>
	';

  header("Content-Type: application/vnd.ms-excel");
  header("Content-Disposition: attachment; filename=emarket.xls");
	...
	$productq = $db->query(
		...
		<table border="1" cellspacing="0" cellpadding="0" style="width:100%;">
		 ...
		 foreach($products as $product){
			 ...
			 $form .=  '<tr>
		       ...
		       <td><center>'.$product['Category_name'].'</center></td>
				   <td><center>'.'php '.$product['old price'].'</center></td>
		       <td><center>'.'php '.$product['new price'].'</center></td>
					 	  <td><center>'.'php '.$totalold.'</center></td>
					  <td><center>'.'php '.$totalnew.'</center></td>
		       <td><center>'.'php '.$profit.'</center></td>
		   </tr>';
		 }
	 	...
	 	<tr>
	 		...
	 		<td colspan="1"><center>'.'php'.' '.$totaloldcount.'</center></td>
	 		<td colspan="1"><center>'.'php'.' '.$totalnewcount.'</center></td>
	 		<td colspan="1"><center>'.'php'.' '.$totalprofit.'</center></td>
	 	</tr>
	 </table>
	 ';
		echo $form;
